...
'''
Summary : For this file we called the washroom detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/washroom.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Washroom_Detection')


def washroom(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = washroom(image, modelpath)
                class_names = ["bucket", "toothbrush", "toothpaste", "washbasin", "trashcan", "mug", "soapdispenser", "westerncommode", "sink", "soap"]
