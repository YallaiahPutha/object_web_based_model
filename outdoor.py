...
'''
Summary : For this file we called the outdoor detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/outdoor.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Outdoor_Detection')


def outdoor(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = outdoor(image, modelpath)
                class_names = ["gate", "bicycle", "plant", "tree", "stairs", "motorcycle", "bench", "car", "trolleybag", "sculpture", "autorikshaw", "bus", "railtrack", "shoppingtrolley", "tankerlorry", "aeroplane", "manhole", "directionsignboard", "tractor", "heavytruck", "callingbell", "signboard"]
                ...
                if index < len(boxes):
                    ...
                    cv2.putText(image, class_names[classes[index]], (xmin, ymin+30), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
