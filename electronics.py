...
'''
Summary : For this file we called the electronics detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/electronic.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Electronics_Detection')

# st.markdown("Welcome to this simple web application that classifies currency. The currency are classified into six different classes namely: 100dollars,10dollars,1dollar,20dollars,50dollars,5dollars ")
# st.markdown("Welcome to this simple web application that classifies currency")


def electronics(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = electronics(image, modelpath)
                class_names = ["mobilephone", "computermouse", "keyboard", "mobilecharger", "earphone", "laptop", "remote", "television", "tubelight", "watches", "wallbulb", "ceilingfan", "wallclock", "refrigerator", "fan", "tablefan", "airconditioner"]
                scores_indices = [idx for idx, score in enumerate(scores) if score > 0.60]
                ...
                if index < len(boxes):
                    ...
                    cv2.putText(image, class_names[classes[index]], (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
