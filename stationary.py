...
modelpath = "./tflite_models/stationary.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Stationary_Detection')


def stationary(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = stationary(image, modelpath)
                class_names = ["pen", "eraser", "protractor", "book", "tape", "scale", "paper", "keys", "calander", "penstand", "fiverupeecoin", "tworupeecoin", "stapler", "calculator", "stamppad", "binderclips", "paperclip", "paperweight", "onerupeecoin", "marker", "sketchpen", "chalkpeice", "duster", "file", "globe", "colourcrayons", "staplerpins", "tenrupeecoin", "stickynotes"]
