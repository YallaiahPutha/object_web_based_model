...
'''
Summary : For this file we called the household detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/household.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Household_Detection')


def household(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = household(image, modelpath)
                class_names = ["blanket", "bed", "chair", "cushion", "sofa", "door", "curtain", "stool", "cupboard", "switchboard", "drawer", "window", "glassbottle", "carrybag", "pillow", "sofachair", "cartonbox", "doorhandle", "glassbowl", "tvcabinet", "glass", "idol", "gaslighter", "shoestand", "diningtable", "glasstable", "ledpanellight", "wineglass", "bookshelf", "handwovencot"]
