...
'''
Summary : For this file we called the fashionwear detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/fashion.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Fashionwear_Detection')


def fashion(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = fashion(image, modelpath)
                class_names = ["spectacle", "facemask", "slippers", "hairband", "helmet", "handbags", "bangle", "dresses", "comb", "hairclip", "waterbottles", "wallets", "belts", "shirts", "tshirts", "ring", "earrings", "nailpolish", "umbrellas", "socks", "caps", "jeans", "canestick", "sportsshoes", "formalshoes"]
