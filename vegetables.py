...
'''
Summary : For this file we called the vegetables detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/vegetables.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Vegetables_Detection')


def vegetables(image_path, modelpath):
    """
    ...
    Returns:
        Output (list of coordinates): got output as label index position and bounding box coordinates
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = vegetables(image, modelpath)
                class_names = ["ladiesfinger", "potatoes", "tomato", "greencapsicum", "brinjal", "onions", "garlic", "greenchili", "leafyvegetables", "ridgegourd", "greenbeans", "broadbeans", "curryleaves", "bittergourd", "bottlegourd", "pumpkin", "cucumber", "radish", "gingerroot", "drumsticks", "clusterbeans", "carrot", "redchilli", "corianderleaves", "broccoli", "kidneybeans"]
