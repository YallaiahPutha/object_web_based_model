...
'''
Summary : For this file we called the kitchenware detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/kitchenware.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Kitchenware_Detection')


def kitchenware(image_path, modelpath):
    """
    ...
    if class_btn:
        if file_uploaded is None:
            ...
            with st.spinner('Model working....'):
                plt.imshow(image)
                plt.axis("off")
                boxes, scores, classes, imH, imW, image_rgb = kitchenware(image, modelpath)
                class_names = ["foodcontainer", "cooker", "jar", "steelbowl", "pot", "spoon", "knife", "fork", "cup", "cremaicplates", "cremaicbowl", "glassbowl", "steelplates", "microwaveoven", "mixiegrinder", "tray", "fryingpan", "choppingboard", "gasstove", "lpgcylinder", "tap", "inductionstove", "thermoflask", "hotbox", "jug", "gaslighter", "lunchbox", "chapatistick", "nailcutter", "kitchencabinet", "kettle"]
                ...
                if index < len(boxes):
                    ...
                    cv2.putText(image, class_names[classes[index]], (xmin+20, ymin+40), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
