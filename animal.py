...
'''
Summary : For this file we called the animal detection model and got output as class_name and bounding_boxes
'''
...
modelpath = "./tflite_models/animal.tflite"

with open("custom.css") as f:
    st.markdown(f"<style>{f.read()}</style>", unsafe_allow_html=True)

st.title('Animals_Detection')


def animals(image_path, modelpath):
    """
        ...
        if class_btn:
            if file_uploaded is None:
                ...
                with st.spinner('Model working....'):
                    plt.imshow(image)
                    plt.axis("off")
                    boxes, scores, classes, imH, imW, image_rgb = animals(image, modelpath)
                    ...
                    if index < len(boxes):
                        ...
                        cv2.putText(image, class_names[classes[index]], (xmin, ymin+20), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36, 255, 12), 2)
                        st.write(class_names[classes[index]])
                        st.image(image, caption='Proccesed Image.')
                        cv2.waitKey(0)
                    else:
                        st.write("No Prediction Found")
